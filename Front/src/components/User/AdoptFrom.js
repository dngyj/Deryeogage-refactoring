...
function AdoptFrom() {
  ...
  const handleRefundResponsibility = async (boardId, index) => {
    ...
    try {
      ...
      const targetIndex = updatedPrecost.findIndex(
          ...
          adopts.map((adopt, index) => (
            <S.BoardRow className="row item align-items-center " key={index}>
              ...
              <S.TitleLink
                ...
                to={`/adopt/${adopt.boardId}`}
              >
                {adopt.boardInfo?.title}
              </S.TitleLink>
              ...
              {adopt.fromConfirmYn ? (
                <div className="col-3 d-flex justify-content-center">
                  <S.AdoptComfirmed
                    ...
                    disabled={
                      precost.find((item) => item.boardId === adopt.boardId)
                        ?.returnYn
                    }
                  >
                    {precost.find((item) => item.boardId === adopt.boardId)
                ...
                <div className="col-3 d-flex justify-content-center">
                  <S.AdoptComfirm
                    className="col-3 text-center"
                    onClick={() =>
                      handleConfirmAdoption(adopt.id, adopt.toUserId, index)
                    }
                  >
                    분양 확정하기
                  </S.AdoptComfirm>
